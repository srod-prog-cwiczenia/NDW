...
\end{observation}
\begin{proof}
For the case of the Furstenberg's topology it suffices to observe that for any $\{an + b\}\in\mathcal{B}_F$ we have: $\{2an + b\} \subseteq \{an + b\}$, $\{2an + a + b\} \subseteq \{an + b\}$, and $\{2an + b\} \cap \{2an + a + b\} = \emptyset$. Moreover, we assume that $b\leq a$, so $\{2an + b\}\in\mathcal{B}_F$ as $b\leq a \leq 2a$, and $\{2an + a + b\}\in\mathcal{B}_F$ as $a+b\leq a+a = 2a$.
...
Now, let us consider the case of the Golomb's topology. For any $\{an + b\}\in\mathcal{B}_G$ we can choose such $p\in\mathbb{P}$ that $p\nmid b$ and $p\nmid a+b$. We have: $\{pan + b\} \subseteq \{an + b\}$, $\{pan + a + b\} \subseteq \{an + b\}$, and $\{pan + b\} \cap \{pan + a + b\} = \emptyset$ (if for some $n_1,n_2\in\N$ there would be $pan_1+b=pan_2+a+b$, then $pa(n_1-n_2)=a$, thus $p(n_1-n_2)=1$ -- a contradiction). Moreover, we assume that $(a,b)=1$, so $\{pan + b\}\in\mathcal{B}_G$ as $(pa,b)=1$, and $\{pan + a + b\}\in\mathcal{B}_G$ as $(a,b)=1 \Rightarrow (a,a+b)=1 \Rightarrow (pa,a+b)=1$.
