...
We use the symbol $(a,b)$ to denote the greatest common divisor of $a$ and $b$. Moreover, the letter $\mathbb{P}$ symbolizes the set of all prime numbers and $\Theta(a)$ stands for the set of all prime factors of $a\in\N$.
...
The following simple examples show that in some way the Furstenberg's ideal significantly differs from the other ideals (namely: from the Golomb's and the Kirch's ideal.)
...
While searching for an example distinguishing these ideals, at first we have obtained a partial result, suggesting that an example of a set witnessing the lack of inclusion between those ideals cannot be found among the sets from $\B_G \setminus \B_K$:
...
Eventually, we managed to prove an inclusion between the ideals of Golomb and Kirch -- however, they are not the same. These results will be shown in the next two theorems. In their proofs we will use a technical lemma:
 
\begin{lem} \label{lemCRT}
Assume that $a,b,a_1,b_1,a_2,b_2 \in\N$. If $\{a_1 n+b_1\}\subseteq \{an+b\}$, $\{a_2 n+b_2\}\subseteq \{an+b\}$ and $(\frac{a_1}{a},\frac{a_2}{a})=1$, then the intersection $\{a_1 n+b_1\}\cap\{a_2 n+b_2\}$ is nonempty (and hence it is an arithmetic progression). 
\end{lem}
%(see: Lemma \ref{lemCRT})
\begin{proof}
Let $f : \N\to \{an+b\}$ be a bijection such that $f(n) = an+b$. Then: 
$$f^{-1}[\{a_1 n+b_1\}] = \{\frac{a_1}{a} n+\frac{b_1-b}{a}\},$$
$$f^{-1}[\{a_2 n+b_2\}] = \{\frac{a_2}{a} n+\frac{b_2-b}{a}\},$$ 
and since $(\frac{a_1}{a},\frac{a_2}{a})=1$, the Chinese remainder theorem guarantees that their intersection is nonempty. Thus,
...
\begin{proof}
We will show that $X \not\in \I_G \implies X \not\in \I_K$.\\
Suppose that $X \not\in \I_G$. Then there exists $\{an+b\}\in \B_G$ such that for every $\{cn+d\}\subseteq \{an+b\}$ with $\{cn+d\}\in \T_G$ we have: $X\cap \{cn+d\} \neq \emptyset$.
...
Note that $\{an+b\}\subseteq \{a'n+b'\}$, $\{c'n+d'\}\subseteq \{a'n+b'\}$, and observe that $(\frac{a}{a'}, \frac{c'}{a'}) = 1$ -- the only prime factors of $\frac{a}{a'}$ are those that have already appeared in the factorization of $a'$, whereas all prime factors of $\frac{c'}{a'}$ must be different from the prime factors of $a'$ since $c'$ is square-free. 
...
Hence, $A:= \{an+b\}\cap\{c'n+d'\} \neq \emptyset$ (by Lemma \ref{lemCRT}). Moreover, $A$ is an arithmetic progression, it belongs to $\T_G$ (as the intersection of two open sets in Golomb's topology), and, obviously, $A \subseteq \{an+b\}$. From the assumption we know that $A\cap X \neq \emptyset$. Since we also have that $A \subseteq \{c'n+d'\}$, it is clear that $\{c'n+d'\}\cap X \neq \emptyset$.
...
\begin{itemize}
	...
	\item $x_k\in \{2^k n+1\}$.
\end{itemize}
Observe that $C_k \cap \{2^k n+1\}$ is always nonempty (if $C_k = \{a_k n+b_k\}$, then $a_k$ is even and square-free, and $(\frac{a_k}{2},\frac{2^k}{2})=(\frac{a_k}{2},2)=1$, as $\frac{a_k}{2}$ is odd; both $\{a_k n+b_k\}$ and $\{2^k n+1\}$ are subsequences of $\{2n+1\}$, so, by 
...
\end{lem}
\begin{proof}
Suppose that there exists a countable family $\mathcal{F}\subseteq [\omega]^\omega$ such that $\I = \MB(\mathcal{F})$. Then,
...
Let $\mathcal{F}\subseteq [\omega]^\omega$ be a countable family. Suppose that 
$\Forall_{F_1, F_2\in\mathcal{F}}\ \Exists_{G\in\mathcal{F}}\ G\subseteq F_1\cap F_2$.
Moreover, assume that $\mathcal{F}$ has the splitting property (i.e., $\Forall_{F\in\mathcal{F}}\ \Exists_{F_1,F_2\in\mathcal{F}, F_1 \cup F_2 \subseteq F}\ F_1 \cap F_2 = \emptyset$). 
...
To prove the previous theorem we need the following result, which may be also interesting in itself. 
...
\begin{proof} (of Thm \ref{thmFsigma})
...
%Let us notice that it is not clear whether the ideal 
%$\textrm{NULL}$ has the $\mathcal{MBC}$ property.
