...
This is our response to Referee's remarks (and also this is
a list of all changes made to the file). 
We are grateful for pointing to us mistakes and suggestions.

Detailed list of changes made according to the suggestions contained in the reports:
...
\item "(...)the topology with the base $B_k$ is not Kirch's topology(...)". 
There is something wrong in the Referee's argument - you write that the set $\langle 6, 15\rangle$ is open in the topology $B_k$ but it is not open in the topology $B_k$ since in the definition of the topology we require that
...
\item "In the small section titled THREE IDEALS there is a reference to 
$\ll$decent$\gg$(...)" - Changed to "non-trivial".

\item "(...)the three topologies are all Hausdorff(...)it is silly to include 
Proposition 1.1." - we removed Proposition 1.1.

\item "(...)should say that $\ll$Any base for any...$\gg$(...)" -
Changed according to Referee's suggestion.

\item "As such, Theorem 2.9 should be moved up(...)" - We moved Theorem 2.9
up before Example 2.4, also we modified the proof of Example 2.4
such that it is for $I_K$ now.

\item "The proof of Example 2.1(...)" - We applied the Referee's proof here.

\item "In the proof of Example 2.3, the displayed equations are overly detailed(...)"
- We removed the superfluous equations.

\item "At the conclusion of the proof of Example 2.3(...)" and
"(...)And rather than(...)immediate consequence of Dirichlet's Theorem." - 
Changed according to both suggestions.

\item "Assuming that Theorem 2.9 was moved forward as requested, (...) should refer
to $I_K \setminus I_F$(...)" - Changed according to Referee's suggestion.

\item "There doesn't seem to be any motive for including Proposition 2.7."
- we removed Proposition 2.7.

\item "Theorem 2.10 can be strengthened to read(...)" - 
We applied the Referee's proof here.

\item "At this stage, one has established the relationships
(...)in the following diagram. (...) ask the question(...)" -
We added both the diagram and the question.

\item "The first sentence of Section 3(...)" - we omitted
the first sentence of Section 3.

\item "The second sentence of the proof of Theorem 3.6(...)" -
changed according to Referee's suggestion.

\item "In the sentence after Corollary 3.8(...)" - we
added a definition of an $F_{\sigma\delta}$ ideal.

\item "In Definition 4.1, $\sigma$ - ideal is not defined(...)
Similarly,(...)selector is not defined.(...)" - We
added both definitions.
 
\item "In [8] NWD(Q)(...)" - We changed according to Referee's suggestion.

\item "In the definition of NWD and NULL(...)
I presume the authors wanted to have $b$ as a 
bijection between(...)" - We changed according
to this suggestion.

\item "On page $15$ (...) It follows from how (...)splits."
- We corrected the formulation according to this remark.
